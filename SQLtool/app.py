...
class DB2ToPostgresConverterApp:
    ...
    def explain_db2_procedure(self, db2_code):
        system_message = """
            ...
            Format the response in a very basic HTML with titles, subtitles, and bullet points for key steps. (for a dark themed background)
            """
        user_message = f"###DB2 Stored Procedure\n{db2_code}"
        return self.mistral_chat(system_message, user_message)
    ...
    def compare_procedures(self, db2_code, postgres_code):
        system_message = """
            ...
            Format the response in a very basic HTML, highlighting syntax differences and logical steps. Finally, Emphasize and give a brief if the result would vary between the two codes. Tabulate, if needed. (for a dark themed background)
            """
        user_message = (
...
def display_outs(postgres_code, explanation, comparison):
    # Display results
...
with tab1:
    ...
    if st.button("Convert and Analyze", key="convert_button_text") and db2_code_input:
        ...
        display_outs(postgres_code, explanation, comparison)
